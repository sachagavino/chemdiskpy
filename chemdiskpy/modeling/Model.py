...
class Model:
    ...
    def run_thermal_radmc3d(self, nphot=1e6, verbose=True, timelimit=7200, \
        ...
        #     self.grid.temperature[i] = self.grid.temperature[i].reshape((n3, n2, n1))
    ...
    def write_nautilus(self, uv_ref=3400, nH_to_AV_conversion=1.600e+21, rgrain=0.1, dtogas=1e-2, ref_radius=100,\
                       static=True, param=True, element=True, abundances=True, \
                       activ_energies=True, surfaces=True,  network=True, \
                       coupling_temp=True, coupling_av=True, **keywords):
        chemfold = 'chemistry'
        if not os.path.exists(chemfold):
        ...
        for idx, r in enumerate(self.grid.rchem):
            ...
            if param == True:
                nautilus.write.parameters(path, resolution=self.grid.nz_chem, uv_flux=uvflux, **keywords)
            if static == True:
