...
def avnh_factor(nH_to_AV_conversion, dtogas, rgrain, z): # rgrain provided in micron
    av_nh = (1/nH_to_AV_conversion)*(dtogas/1e-2)*(1e-5/(rgrain*1e-4))*np.ones(len(z))
    return av_nh

def static(path, z, hg, gas_density, T_gas, av_z, T_dust, dust_density, r_grain, avnh_fact):
    distance = z*hg
    nh = gas_density
    Tgas = T_gas
    avz = av_z
    ...
    static_array = np.stack((distance, nh, Tgas, avz, diff_coef, Tdust, inv_ab, avnhfact, rgrain), axis=-1)
    header_static = "z [AU] ; H Gas density [part/cm^3] ; Tgas [K] ; Av [mag] ; Diffusion coef [cm^2/s]; Tdust [K]; 1/ab of grains ; AV/NH conversion factor ; Grain radius (cm)"
